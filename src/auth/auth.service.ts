...
import { LoginInput, RegisterInput, ResendOtpInput, VerifyOtpInput } from './dto';
...
export class AuthService {
	...
	async verifyOtp(dto: VerifyOtpInput): Promise<MessageResponseDto> {
	...
	async resendOtp(dto: ResendOtpInput): Promise<MessageResponseDto> {
		const user = await this.prisma.user.findUnique({
			where: { email: dto.email },
			include: { credentials: true }
		});
